import configparser
import json
from textblob import TextBlob # nltk wrapper
import random
import logging
import os
...
emotion_path = config['file_path']['emotion_corpus']
food_path = config['file_path']['food_corpus']

# Read in Corpus
# ---------------------------------------------------------
greeting_corpus = json.load(open(greeting_path))
emotion_corpus = json.load(open(emotion_path))
food_corpus = json.load(open(food_path))

# Construct greetings by extracting key, values from the
# corpus
# ---------------------------------------------------------
greeting = greeting_corpus['greetings']
greeting_dict = {}

for x in range(0,len(greeting)):
    greeting_key = greeting[x][0].lower()
    greeting_value = greeting[x][1]
    greeting_dict[greeting_key] = greeting_value

# function to check for greeting and return a valid response
# ---------------------------------------------------------
def check_for_greeting(sentence):
    """If any of the words in the user's input was a greeting, return a greeting response"""
    for word in sentence.words:
        if word.lower() in greeting_dict.keys():
            valid_responses = greeting_dict[word.lower()]
